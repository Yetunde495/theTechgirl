...
export const LogoCard = ({
  ...
  return (
    <div
      className={cn(
        "flex gap-2 py-2 items-center px-3 shrink-0 cursor-pointer overflow-hidden hover:scale-110 rounded-xl border border-transparent transition-all hover:border-blue-400 hover:shadow-[0_0_10px_#60a5fa] dark:hover:border-blue-400",
        className
      )}
    >
      <div className="w-8 h-8 flex items-center justify-center">
        {SvgIcon ? (
          <SvgIcon className="h-full w-full object-cover" />
        ) : (
          <img
            src={logo.image}
            alt={logo.name}
